...
<template>
  <nav class="flex justify-between items-center p-4 bg-gray-900 text-white shadow-md">
    ...
    <div v-if="route.path !== '/login'">
      <div v-if="isUserFetching" class="text-gray-400 text-center">Loading...</div>
      <div v-else class="flex items-center space-x-4">
        <div v-if="user" id="user-dropdown" class="relative">
          <div
            ...
            <span class="hidden sm:block">{{ user?.displayName || 'User' }}</span>
          </div>
          <div
            v-if="isDropdownOpen"
        ...
        <div v-else>
          <button
            class="bg-gray-700 font-bold hover:bg-gray-800 text-white px-6 rounded-lg transition"
